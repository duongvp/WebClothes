...
function QuickView()
{
    $('.btn-quickview').click(function(event) {
        /* Act on the event */
        debugger
        id=$(this).attr('data-btn');
        txt=$('#pro-'+id).find('#price').text();
        proname=$('#pro-'+id).find('.product-name').text();
        img=$('#pro-'+id).find('.img').attr('src');
        console.log(img);
        $('.img-aolen-parent .img-src').attr('src',img);
        $('.quickview-content .product-name').text(proname);
        $('.quickview-content #price').text(txt);
